               ...
               <form onSubmit={(e) => { e.preventDefault(); const fd = new FormData(e.currentTarget); 
                  ...
                  {modalMode === 'resource' && (
                     <>
                        ...
                        {resFormType === 'text' && <div><label className="block text-xs font-bold text-slate-500 mb-1">Contenu</label><textarea value={resFormContent} onChange={(e) => setResFormContent(e.target.value)} placeholder="Article..." rows={6} className="w-full border p-2 rounded font-mono text-sm" /></div>}
                  ...
                  {modalMode === 'user' && (
                    <>
                        ...
                        <div>
                            ...
                            <select value={userFormStructure} onChange={e=>setUserFormStructure(e.target.value)} className="w-full border p-2 rounded">
